...
| `YOUTUBE_CLIENT_SECRETS_JSON` | YouTube OAuth JSON (one line) — can be same or separate Google client |
...
Add to **Authorized redirect URIs** (YouTube / CreatorOS client):
...
| YouTube OAuth fails | Set `YOUTUBE_OAUTH_REDIRECT_URI` and add `/api/youtube/callback` in Google Console; enable YouTube Data API v3 |
