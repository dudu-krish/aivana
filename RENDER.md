...
| Variable | Example |
|----------|---------|
| `APP_BASE_URL` | `https://aivana-65kg.onrender.com` |
| `OAUTH_REDIRECT_URI` | `https://aivana-65kg.onrender.com/api/gmail/callback` |
| `YOUTUBE_OAUTH_REDIRECT_URI` | `https://aivana-65kg.onrender.com/api/youtube/callback` |
| `GOOGLE_CLIENT_SECRETS_JSON` | Gmail OAuth JSON (one line) — **not** for YouTube |
| `YOUTUBE_CLIENT_SECRETS_JSON` | **Required for YouTube.** Paste `credentials/youtube_client_secret.json` (different OAuth client than Gmail) |

`APP_SECRET` is auto-generated by the blueprint.

### 3. Google Cloud OAuth

Add to **Authorized redirect URIs** (Gmail client):

```text
https://aivana-65kg.onrender.com/api/gmail/callback
```

Add to **Authorized redirect URIs** (YouTube OAuth client — client id `...fenfc1s9ed...`, **not** the Gmail client):

```text
https://aivana-65kg.onrender.com/api/youtube/callback
```
...
- YouTube: `youtube.readonly`, `youtube.upload`, `yt-analytics.readonly`
...
| `RENDER_SERVICE_URL` | Your public URL, e.g. `https://aivana-65kg.onrender.com` |
...
| YouTube OAuth fails | Set `YOUTUBE_CLIENT_SECRETS_JSON` on Render (separate from Gmail). Verify `/api/youtube/setup` shows the YouTube client id, not the Gmail one |
